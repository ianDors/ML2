...
coef(summary(fit))
```
Use 'lm' function to create a linear model that predicts wage using a fourth-degree polynomial of age. *Each column in returned matrix is a linear combination of the variables age, age^2, age^3, age^4.???
*Contains four terms: age, age^2, age^3, age^4.
estimated four parameters (beta coeff)
```{r, echo=FALSE}
...
Raw means non-orthongianl
```{r}
fit2a = lm(wage~age+I(age^2)+I(age^3)+I(age^4), data=Wage)
coef(fit2a)
```
I() isolates features to create the same estimates as using 'raw = True'.
